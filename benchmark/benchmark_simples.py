...
class BenchmarkSimples:
    def __init__(self, path_zip, path_base, arquivo_zip_especifico=None, path_parquet_destino=None,
    ...
    def executar_benchmark_pandas(self):
        ...
        self.resultados['pandas']['memoria_media'] = sum(memoria_medidas) / len(
            memoria_medidas) if memoria_medidas else 0
            ...
            for i, zip_file in enumerate(self.zip_files):
                ...
                for tentativa in range(3):  # Até 3 tentativas por arquivo
                    ...
                    try:
                        ...
                        logger.debug(
                            f"Arquivo processado em {tempo_processamento_arquivo:.2f}s, resultado: {resultado}")
    ...
    def executar_benchmark_polars(self, max_workers=None):
        ...
        espaco_disco = sum(os.path.getsize(os.path.join(parquet_dir, f)) for f in parquet_files) / (
                    1024 * 1024) if parquet_files else 0
    ...
    def comparar_resultados(self):
        ...
        if not comparacao:
            return {
                'comparacao': {'sem_dados': {'melhor': 'indeterminado', 'diferenca_percentual': 0}},
                'contagem_vitorias': {'pandas': 0, 'dask': 0, 'polars': 0}, # Renomeado
                'pontuacoes_ponderadas': {'pandas': 0, 'dask': 0, 'polars': 0}, # Adicionado
                'melhor_metodo': 'indeterminado'
            }
        ...
        return {
            ...
            'contagem_vitorias': contagem, # Renomeado
            'pontuacoes_ponderadas': pontuacoes, # Adicionado
    ...
    def gerar_graficos(self):
        ...
        try:
            ...
            for i, metrica in enumerate(metricas):
                plt.subplot(2, 2, i + 1)
                barras = plt.bar(['Pandas', 'Dask', 'Polars'],
                                 [self.resultados['pandas'][metrica], self.resultados['dask'][metrica],
                                  self.resultados['polars'][metrica]],
    ...
    def imprimir_relatorio(self):
        ...
        for zip_file, info in self.info_arquivo.items():
            print(
                f"  - {zip_file}: {info['tamanho_zip_mb']:.1f} MB (ZIP), {info['tamanho_real_mb']:.1f} MB (extraído), {info['num_csv']} CSV")
