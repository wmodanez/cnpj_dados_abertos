    ...
    try:
        ...
        if config.cache.enabled:
            if final_local_size == expected_size:
                ...
                logger.error(
                    f"Tamanho final do arquivo {filename} ({final_local_size}) difere do remoto esperado ({expected_size}). Cache não atualizado.")
                ...
                if attempt_resume:
                    if response.status == 206:
                        ...
                        async with session.get(url, timeout=aiohttp.ClientTimeout(total=None, sock_connect=30,
                                                                                  sock_read=300)) as response_fallback:
