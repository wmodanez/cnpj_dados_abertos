...
def process_single_zip(zip_file: str, path_zip: str, path_unzip: str, path_parquet: str, 
    ...
    try:
        ...
        logger.debug(f"[{pid}] Fase 2: Iniciando leitura e processamento de {len(data_files)} arquivos de dados...")
        
        logger.info(f"[{pid}] Processando {len(data_files)} arquivos de dados extraídos")
...
def process_empresa_files(path_zip: str, path_unzip: str, path_parquet: str, create_private: bool = False) -> bool:
    """
    Processa todos os arquivos de empresa encontrados no diretório ZIP.
    """
    # Log detalhado dos recursos do sistema
    log_system_resources_empresa()
    
    logger.info(f"Iniciando processamento de arquivos de empresa em {path_zip}")
    ...
    try:
        ...
        progress_tracker.register_module("empresas", len(zip_files), _max_concurrent_processes.value)
        ...
        progress_tracker.print_final_summary("empresas")
        ...
        progress_tracker.cleanup("empresas")
...
def log_system_resources_empresa():
    """Log detalhado dos recursos do sistema para processamento de empresas."""
    ...
    logger.info("=" * 50)
    logger.info("🏢 MÓDULO EMPRESA - CONFIGURAÇÃO DE RECURSOS")
    logger.info("=" * 50)
